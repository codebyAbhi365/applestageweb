...
export default function Stage1() {
  ...
  const handleSpeak = () => {
    ...
    const textToSpeak = `${t('stage1_overviewTitle')}. ${t('stage1_overviewDesc')}. ${t('stage1_preventionTitle')}. ${t('stage1_preventionDesc')}. ${t('stage1_actionTitle')}. ${t('stage1_actionSteps', { returnObjects: true }).join('. ')}`;
  ...
  const modelPath = "/models/apples1.glb";
  ...
  return (
    <div className="min-h-screen font-sans bg-rose-50 text-slate-800">
      <header className="sticky top-0 z-50 flex items-center justify-between p-4 border-b bg-white/80 backdrop-blur-lg border-rose-100">
        ...
        <h1 className="text-lg font-bold text-slate-700">{t('stage1_title')}</h1>
      ...
      <main className="px-4 py-6 md:px-6 md:py-8">
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="p-5 border bg-white/70 backdrop-blur-md rounded-2xl border-rose-100">
            <div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 flex items-center justify-center bg-rose-100 rounded-full text-[#E06B80]"><Leaf size={20} /></div><h2 className="text-lg font-bold text-slate-700">{t('stage1_overviewTitle')}</h2></div>
            <p className="leading-relaxed text-slate-600">{t('stage1_overviewDesc')}</p>
          </div>
          <div className="p-5 border bg-white/70 backdrop-blur-md rounded-2xl border-rose-100">
            <div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 flex items-center justify-center bg-rose-100 rounded-full text-[#E06B80]"><ShieldAlert size={20} /></div><h2 className="text-lg font-bold text-slate-700">{t('stage1_preventionTitle')}</h2></div>
            <p className="leading-relaxed text-slate-600">{t('stage1_preventionDesc')}</p>
          </div>
          <div className="p-5 border bg-white/70 backdrop-blur-md rounded-2xl border-rose-100">
            <div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 flex items-center justify-center bg-rose-100 rounded-full text-[#E06B80]"><ListChecks size={20} /></div><h2 className="text-lg font-bold text-slate-700">{t('stage1_actionTitle')}</h2></div>
            <ul className="space-y-2 list-disc list-inside text-slate-600">
              {t('stage1_actionSteps', { returnObjects: true }).map((step, index) => (<li key={index}>{step}</li>))}
            </ul>
          </div>
        </section>
      </main>
